...
fn main() -> Result<(), Box<std::error::Error>> {
    ...
    let f = std::fs::File::open(&args.appfile).expect(&format!("Cannot open file: {}",&args.appfile.as_path().display().to_string()));
    appmgr.add_app(f).unwrap();

    println!("{}",appmgr.get_app(0).general.name);
